...
def test_detects_every_minute_cron(tmp_path: Path) -> None:
    write(tmp_path, "ci.yml", "on:\n  schedule:\n    - cron: '* * * * *'\n")
    assert "GL003" in rule_ids(scan([str(tmp_path)]))


def test_detects_select_star_and_fat_base(tmp_path: Path) -> None:
...
def test_slim_base_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "Dockerfile", "FROM debian:bookworm-slim\n")
    assert "GL006" not in rule_ids(scan([str(tmp_path)]))


def test_findings_sorted_high_first(tmp_path: Path) -> None:
...
def test_missing_config_is_a_noop(tmp_path: Path) -> None:
    assert load_config(str(tmp_path / "nope.toml")) == {"disable": set(), "ignore": []}


def test_ast_busy_loop_ignores_unrelated_sleep_elsewhere_in_file(tmp_path: Path) -> None:
...
def test_ast_busy_loop_not_flagged_when_loop_actually_sleeps(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "def poll():\n    while True:\n        time.sleep(1)\n")
    assert "GL001" not in rule_ids(scan([str(tmp_path)]))


def test_paginator_loop_not_flagged(tmp_path: Path) -> None:
...
def test_read_until_eof_loop_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
        "def f(conn):\n    while True:\n        ch = conn.recv(1)\n        if not ch:\n            break\n",
...
def test_loop_returning_a_value_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
        "def f(it):\n    while True:\n        n = next(it)\n        if n > end:\n            return n\n",
...
def test_break_in_a_nested_loop_does_not_exonerate(tmp_path: Path) -> None:
...
def test_ast_busy_loop_respects_disable(tmp_path: Path) -> None:
...
def test_findings_carry_a_co2e_estimate(tmp_path: Path) -> None:
    write(tmp_path, "q.sql", "SELECT * FROM users;\n")
    findings = scan([str(tmp_path)])
    assert findings[0]["co2e_estimate"]


def test_cli_github_format_emits_workflow_commands(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
    write(tmp_path, "q.sql", "SELECT * FROM users;\n")
    ...
    assert "file=" in out
    assert "line=" in out
    assert "title=greenlint" in out


def test_detects_apt_and_pip_without_cleanup_flags(tmp_path: Path) -> None:
    write(
        tmp_path,
        "Dockerfile",
...
def test_apt_and_pip_cleanup_flags_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "Dockerfile",
        "FROM debian:bookworm-slim\n"
    ...
    ids = rule_ids(scan([str(tmp_path)]))
    assert "GL009" not in ids
    assert "GL010" not in ids


def test_detects_img_missing_lazy_loading(tmp_path: Path) -> None:
    write(tmp_path, "page.html", '<img src="hero.png">\n')
    assert "GL011" in rule_ids(scan([str(tmp_path)]))


def test_img_with_loading_attr_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "page.html", '<img loading="lazy" src="hero.png">\n')
    assert "GL011" not in rule_ids(scan([str(tmp_path)]))


def test_detects_query_in_loop(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
...
def test_batched_query_not_flagged_as_n_plus_1(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
...
def test_detects_s3_bucket_without_lifecycle(tmp_path: Path) -> None:
    write(
        tmp_path,
        "bucket.tf",
...
def test_s3_bucket_with_lifecycle_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "bucket.tf",
        'resource "aws_s3_bucket" "logs" {\n  bucket = "my-logs"\n  lifecycle_rule {\n    enabled = true\n  }\n}\n',
...
def test_detects_k8s_deployment_without_resources(tmp_path: Path) -> None:
    write(
        tmp_path,
        "deploy.yaml",
        "apiVersion: apps/v1\nkind: Deployment\nspec:\n"
...
def test_k8s_deployment_with_resources_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "deploy.yaml",
        "apiVersion: apps/v1\nkind: Deployment\nspec:\n"
...
def test_detects_eol_docker_base_image(tmp_path: Path) -> None:
    write(tmp_path, "Dockerfile", "FROM node:12\n")
    assert "GL015" in rule_ids(scan([str(tmp_path)]))


def test_current_docker_base_image_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "Dockerfile", "FROM node:22\n")
    assert "GL015" not in rule_ids(scan([str(tmp_path)]))


def test_detects_non_graviton_instance_family(tmp_path: Path) -> None:
    write(
        tmp_path,
        "main.tf",
...
def test_graviton_instance_family_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "main.tf",
...
def test_detects_gif_in_html_and_css(tmp_path: Path) -> None:
...
def test_png_in_html_and_css_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "page.html", '<img src="hero.png">\n')
    write(tmp_path, "style.css", '.bg { background: url("loading.png"); }\n')
    assert "GL017" not in rule_ids(scan([str(tmp_path)]))


def test_detects_nested_loop_over_same_collection(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
        "def f(items):\n    for i in items:\n        for j in items:\n            pass\n",
...
def test_nested_loop_over_different_collections_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
        "def f(items, others):\n    for i in items:\n        for j in others:\n            pass\n",
...
def test_detects_http_request_in_loop(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
        "def f(ids):\n    for i in ids:\n        requests.get(f'/x/{i}')\n",
...
def test_batched_http_request_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
...
def test_detects_eager_logging_fstring(tmp_path: Path) -> None:
    write(tmp_path, "a.py", 'import logging\ndef f(x):\n    logging.debug(f"x={x}")\n')
    assert "GL020" in rule_ids(scan([str(tmp_path)]))


def test_lazy_logging_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "a.py", 'import logging\ndef f(x):\n    logging.debug("x=%s", x)\n')
    assert "GL020" not in rule_ids(scan([str(tmp_path)]))


def test_detects_pandas_iterrows(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
...
def test_vectorised_pandas_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "a.py", 'def f(df):\n    df["c"] = df["a"] + df["b"]\n')
    assert "GL021" not in rule_ids(scan([str(tmp_path)]))


def test_detects_file_open_in_loop(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "def f(paths):\n    for p in paths:\n        f = open(p)\n")
    assert "GL022" in rule_ids(scan([str(tmp_path)]))


def test_file_opened_outside_loop_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "def f(paths):\n    data = [open(p).read() for p in paths]\n")
    assert "GL022" not in rule_ids(scan([str(tmp_path)]))


def test_detects_manual_bubble_sort(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
        "def f(a):\n    n = len(a)\n    for i in range(n):\n"
...
def test_builtin_sorted_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "def f(a):\n    return sorted(a)\n")
    assert "GL023" not in rule_ids(scan([str(tmp_path)]))


def test_detects_fixed_size_autoscaling_group(tmp_path: Path) -> None:
    write(
        tmp_path,
        "asg.tf",
...
def test_elastic_autoscaling_group_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "asg.tf",
...
def test_detects_gp2_volume(tmp_path: Path) -> None:
    write(
        tmp_path,
        "vol.tf",
...
def test_gp3_volume_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "vol.tf",
        'resource "aws_ebs_volume" "v" {\n  volume_type = "gp3"\n}\n',
...
def test_detects_log_group_without_retention(tmp_path: Path) -> None:
    write(
        tmp_path,
        "log.tf",
...
def test_log_group_with_retention_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "log.tf",
...
def test_single_dockerfile_install_layer_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "Dockerfile",
...
def test_detects_dict_items_discarding_key_or_value(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "def f(d):\n    for _, v in d.items():\n        print(v)\n")
    assert "GL030" in rule_ids(scan([str(tmp_path)]))


def test_dict_items_using_both_key_and_value_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "def f(d):\n    for k, v in d.items():\n        print(k, v)\n")
    assert "GL030" not in rule_ids(scan([str(tmp_path)]))


def test_detects_lookup_guarded_by_except_in_loop(tmp_path: Path) -> None:
...
def test_detects_int_conversion_used_as_a_type_test(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
        "def f(items):\n    for s in items:\n        try:\n            n = int(s)\n"
...
def test_swallowed_io_error_in_loop_not_flagged(tmp_path: Path) -> None:
...
def test_retry_loop_handler_not_flagged(tmp_path: Path) -> None:
...
def test_per_item_error_collector_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
        "def f(checks):\n    for c in checks:\n        try:\n            evaluate(c)\n"
...
def test_handler_that_breaks_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
...
def test_try_except_around_loop_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
        "def f(keys, d):\n    try:\n        for k in keys:\n            v = d[k]\n    except KeyError:\n        pass\n",
...
def test_detects_malloc_in_loop(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.c",
        "void f(int n) {\n    for (int i = 0; i < n; i++) {\n        int *buf = malloc(100);\n    }\n}\n",
...
def test_malloc_outside_loop_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.c",
...
def test_detects_fixed_range_hpa(tmp_path: Path) -> None:
    write(
        tmp_path,
        "hpa.yaml",
...
def test_elastic_hpa_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "hpa.yaml",
        "apiVersion: autoscaling/v2\nkind: HorizontalPodAutoscaler\nspec:\n  minReplicas: 1\n  maxReplicas: 5\n",
...
def test_config_array_may_span_lines(tmp_path: Path) -> None:
    cfg = tmp_path / ".greenlint.toml"
    cfg.write_text('ignore = [\n  "*/vendor/*",\n  "*/examples/*",\n]\n')
    assert load_config(str(cfg))["ignore"] == ["*/vendor/*", "*/examples/*"]


def test_multiline_ignore_actually_excludes_files(tmp_path: Path) -> None:
    cfg = tmp_path / ".greenlint.toml"
    cfg.write_text('ignore = [\n  "*/examples/*",\n]\n')
...
def test_detects_two_install_layers_in_one_stage(tmp_path: Path) -> None:
    write(
        tmp_path,
        "Dockerfile",
        "FROM alpine\nRUN apk add curl\nRUN pip install foo\nRUN pip install bar\n",
...
def test_install_layers_in_separate_build_stages_not_flagged(tmp_path: Path) -> None:
...
def test_detects_quadratic_list_rebuild(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "out = []\nfor i in range(10):\n    out = out + [i]\n")
    assert "GL007" in rule_ids(scan([str(tmp_path)]))


def test_detects_string_concat_in_loop(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "s = ''\nfor w in words:\n    s += w\n")
    assert "GL007" in rule_ids(scan([str(tmp_path)]))


def test_append_in_loop_is_not_flagged(tmp_path: Path) -> None:
...
def test_counter_increment_in_loop_is_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "total = 0\nfor i in range(10):\n    total += 1\n")
    assert "GL007" not in rule_ids(scan([str(tmp_path)]))


def test_numeric_accumulation_is_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
...
def test_tuple_unpacked_counters_are_not_flagged(tmp_path: Path) -> None:
...
def test_tuple_unpacked_list_still_detects_rebuild(tmp_path: Path) -> None:
...
def test_list_extend_via_augmented_assign_is_not_flagged(tmp_path: Path) -> None:
...
def test_list_extend_with_comprehension_is_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "xs = []\nfor g in groups:\n    xs += [x for x in g]\n")
    assert "GL007" not in rule_ids(scan([str(tmp_path)]))


def test_extend_of_a_name_known_to_be_a_list_is_not_flagged(tmp_path: Path) -> None:
...
def test_bytes_accumulation_of_a_call_result_is_still_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "a.py",
        "def f(conn):\n    data = b''\n    while True:\n        data += conn.recv(10)\n",
...
def test_byte_accumulation_is_still_flagged(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "data = b''\nwhile True:\n    data += chunk\n")
    assert "GL007" in rule_ids(scan([str(tmp_path)]))


def test_rebinding_from_other_names_is_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "for i in range(10):\n    z = x + y\n")
    assert "GL007" not in rule_ids(scan([str(tmp_path)]))


def test_cursor_seeded_from_arithmetic_is_not_flagged(tmp_path: Path) -> None:
...
def test_assignment_of_a_list_expression_still_detects_rebuild(tmp_path: Path) -> None:
...
def test_coordinate_seeded_from_a_parameter_is_not_flagged(tmp_path: Path) -> None:
...
def test_string_accumulator_is_still_flagged_when_the_scope_does_arithmetic(tmp_path: Path) -> None:
...
def test_numeric_evidence_does_not_leak_across_scopes(tmp_path: Path) -> None:
...
def test_helm_template_not_flagged_for_missing_resources(tmp_path: Path) -> None:
    write(
        tmp_path,
        "workload.yaml",
...
def test_sleep_of_exactly_100ms_is_not_flagged(tmp_path: Path) -> None:
    # The rule is "sub-100ms". 0.1s is 100ms.
    write(tmp_path, "a.py", "time.sleep(0.1)\n")
    assert "GL002" not in rule_ids(scan([str(tmp_path)]))


def test_sleep_below_100ms_is_still_flagged(tmp_path: Path) -> None:
    write(tmp_path, "a.py", "time.sleep(0.05)\n")
    assert "GL002" in rule_ids(scan([str(tmp_path)]))


def test_short_sleep_in_a_test_file_is_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "proxy_test.go", "func f() { time.Sleep(10 * time.Millisecond) }\n")
    assert "GL002" not in rule_ids(scan([str(tmp_path)]))


def test_short_sleep_in_production_code_is_still_flagged(tmp_path: Path) -> None:
    write(tmp_path, "proxy.go", "func f() { time.Sleep(10 * time.Millisecond) }\n")
    assert "GL002" in rule_ids(scan([str(tmp_path)]))


def test_pattern_in_a_comment_is_not_a_finding(tmp_path: Path) -> None:
...
def test_pattern_in_a_docstring_is_not_a_finding(tmp_path: Path) -> None:
    write(tmp_path, "a.py", '"""Never write SELECT * FROM orders."""\n')
    assert "GL005" not in rule_ids(scan([str(tmp_path)]))


def test_pattern_in_a_real_string_is_still_a_finding(tmp_path: Path) -> None:
...
def test_block_comment_is_blanked(tmp_path: Path) -> None:
    write(tmp_path, "a.go", "/*\n  SELECT * FROM users\n*/\nfunc f() {}\n")
    assert "GL005" not in rule_ids(scan([str(tmp_path)]))


def test_commented_out_dockerfile_directive_not_flagged(tmp_path: Path) -> None:
    write(tmp_path, "Dockerfile", "# FROM ubuntu:22.04\nFROM alpine:3.24\n")
    assert "GL006" not in rule_ids(scan([str(tmp_path)]))


def test_detects_full_history_clone(tmp_path: Path) -> None:
    write(
        tmp_path,
        "ci.yml",
...
def test_full_history_clone_not_flagged_when_a_scanner_needs_it(tmp_path: Path) -> None:
    write(
        tmp_path,
        "security.yml",
        "jobs:\n  secrets:\n    steps:\n      - uses: actions/checkout@v5\n        with:\n"
...
def test_full_history_clone_in_a_comment_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "action.yml",
...
def test_full_history_clone_not_flagged_for_docs_date_plugins(tmp_path: Path) -> None:
...
def test_full_history_clone_not_flagged_for_super_linter(tmp_path: Path) -> None:
...
def test_full_history_clone_still_flagged_for_a_plain_build(tmp_path: Path) -> None:
...
def test_full_history_clone_not_flagged_for_release_tooling(tmp_path: Path) -> None:
    write(
        tmp_path,
        "release.yml",
        "jobs:\n  release:\n    steps:\n      - uses: actions/checkout@v5\n        with:\n"
...
def test_detects_compose_service_without_limits(tmp_path: Path) -> None:
    write(tmp_path, "docker-compose.yml", "services:\n  web:\n    image: nginx\n")
    assert "GL034" in rule_ids(scan([str(tmp_path)]))


def test_compose_service_with_mem_limit_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "docker-compose.yml",
        "services:\n  web:\n    image: nginx\n    mem_limit: 512m\n",
...
def test_opentofu_files_use_the_same_terraform_rules(tmp_path: Path) -> None:
    write(
        tmp_path,
        "main.tofu",
        'resource "aws_instance" "web" {\n  instance_type = "m5.large"\n}\n',
...
def test_detects_k8s_cronjob_schedule_every_minute(tmp_path: Path) -> None:
    write(
        tmp_path,
        "cron.yaml",
        "apiVersion: batch/v1\nkind: CronJob\nspec:\n  schedule: '* * * * *'\n",
...
def test_detects_sub_100ms_polling_across_languages(tmp_path: Path) -> None:
...
def test_normal_sleep_intervals_across_languages_not_flagged(tmp_path: Path) -> None:
...
def test_select_star_detected_in_c_and_rust(tmp_path: Path) -> None:
...
def test_swift_long_timer_interval_not_flagged(tmp_path: Path) -> None:
    write(
        tmp_path,
        "Poll.swift",
...
def test_apostrophe_does_not_unblank_the_rest_of_the_file(tmp_path: Path) -> None:
...
def test_apostrophe_does_not_hide_a_real_finding(tmp_path: Path) -> None:
    write(tmp_path, "x.sh", "echo don't\nsleep 0.01\n")
    assert "GL002" in rule_ids(scan([str(tmp_path)]))


def test_busy_loop_found_despite_return_in_nested_function(tmp_path: Path) -> None:
...
def test_quadratic_rebuild_name_bindings_are_per_scope(tmp_path: Path) -> None:
...
def test_fetch_depth_exemption_is_per_job(tmp_path: Path) -> None:
    write(
        tmp_path,
        "w.yml",
        "name: demo\n"
...
def test_python_file_is_parsed_once(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    import greenlint
...
def test_multiline_ignore_array_is_parsed(tmp_path: Path) -> None:
    write(tmp_path, "tests/q.sql", "SELECT * FROM users;\n")
    cfg = write(tmp_path, ".greenlint.toml", 'ignore = [\n  "*/examples/*",\n  "*/tests/*",\n]\n')
    assert scan([str(tmp_path)], load_config(str(cfg))) == []


def test_relative_ignore_glob_applies(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_string_instead_of_list_is_rejected(tmp_path: Path) -> None:
    import pytest
...
def test_malformed_config_aborts(tmp_path: Path) -> None:
    import pytest
...
def test_core_seconds_per_gram_anchor() -> None:
...
def test_no_hint_quotes_a_fictional_per_call_gram_figure() -> None:
...
def test_cron_hint_multiplies_out_to_something_sane() -> None:
...
def test_scan_file_accepts_text_instead_of_reading_the_file(tmp_path: Path) -> None:
...
def test_scan_file_text_override_still_picks_language_from_the_path(tmp_path: Path) -> None:
...
def test_iter_files_selects_what_scan_scans(tmp_path: Path) -> None:
...
def test_finding_sort_key_orders_high_severity_first(tmp_path: Path) -> None:
    write(tmp_path, "ci.yml", "cron: '* * * * *'\nfetch-depth: 0\n")
    findings = scan([str(tmp_path)])
...
def test_scannable_matches_whether_scan_file_can_find_anything(tmp_path: Path) -> None:
...
def test_index_python_marks_only_scopes_that_own_a_loop() -> None:
...
def test_blank_comments_preserves_length_and_newlines() -> None:
    text = "a = 1  # one\nb = 2  /* two */ c\n# three\n"
...
def test_blank_spans_returns_the_original_when_there_is_nothing_to_blank() -> None:
    text = "unchanged\n"
...
def test_cli_exclude_flag_skips_matching_paths(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_cli_exclude_is_repeatable_and_adds_to_the_config(tmp_path: Path, capsys: pytest.CaptureFixture[str]) -> None:
...
def test_prunable_bases_only_takes_patterns_that_cover_a_whole_directory() -> None:
...
def test_walk_files_never_descends_into_pruned_directories(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_walk_files_prunes_virtualenvs_and_caches(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_walk_files_matches_what_rglob_selected(tmp_path: Path) -> None:
...
def test_walk_files_survives_an_unreadable_directory(tmp_path: Path) -> None:
...
def test_baseline_accepts_current_findings_and_still_reports_new_ones(
...
def test_fingerprint_is_the_same_for_absolute_and_relative_paths(
...
def test_fingerprint_survives_edits_above_the_finding(tmp_path: Path) -> None:
...
def test_a_missing_or_broken_baseline_reports_everything(tmp_path: Path) -> None:
...
def test_cli_writes_and_then_honours_a_baseline(
...
def test_cli_rejects_a_baseline_path_that_is_not_there(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_every_rule_has_a_matching_heading_in_the_docs() -> None:
...
def test_rule_anchors_are_unique() -> None:
...
class TestCSharpRules:
    def test_new_httpclient_per_call(self, tmp_path: Path) -> None:
        assert "GL039" in scan_one(tmp_path, "a.cs", "var c = new HttpClient();\n")

    def test_a_reused_client_is_not_flagged(self, tmp_path: Path) -> None:
    ...
    def test_blocking_on_a_task(self, tmp_path: Path) -> None:
        ids = scan_one(tmp_path, "c.cs", "var r = FetchAsync().Result;\ntask.Wait();\n")
        assert "GL040" in ids

    def test_await_is_not_flagged(self, tmp_path: Path) -> None:
        assert "GL040" not in scan_one(tmp_path, "d.cs", "var r = await FetchAsync();\n")

    def test_tolist_just_to_iterate(self, tmp_path: Path) -> None:
        assert "GL041" in scan_one(tmp_path, "e.cs", "foreach (var x in items.Where(i => i.Ok).ToList()) { }\n")

    def test_tolist_kept_as_a_variable_is_not_flagged(self, tmp_path: Path) -> None:
...
class TestKotlinRules:
    def test_globalscope(self, tmp_path: Path) -> None:
        assert "GL042" in scan_one(tmp_path, "a.kt", "GlobalScope.launch { work() }\n")

    def test_a_scoped_launch_is_not_flagged(self, tmp_path: Path) -> None:
        assert "GL042" not in scan_one(tmp_path, "b.kt", "viewModelScope.launch { work() }\n")

    def test_filter_map_chain(self, tmp_path: Path) -> None:
        assert "GL043" in scan_one(tmp_path, "c.kt", "val out = xs.filter { it.ok }.map { it.name }\n")

    def test_mapnotnull_is_not_flagged(self, tmp_path: Path) -> None:
        assert "GL043" not in scan_one(tmp_path, "d.kt", "val out = xs.mapNotNull { if (it.ok) it.name else null }\n")

    def test_runblocking(self, tmp_path: Path) -> None:
        assert "GL044" in scan_one(tmp_path, "e.kt", "runBlocking { fetch() }\n")

    def test_a_suspend_call_is_not_flagged(self, tmp_path: Path) -> None:
        assert "GL044" not in scan_one(tmp_path, "f.kt", "suspend fun go() { fetch() }\n")


class TestSwiftRules:
    def test_filter_map_chain(self, tmp_path: Path) -> None:
        assert "GL045" in scan_one(tmp_path, "a.swift", "let out = xs.filter { $0.ok }.map { $0.name }\n")

    def test_compactmap_is_not_flagged(self, tmp_path: Path) -> None:
        assert "GL045" not in scan_one(tmp_path, "b.swift", "let out = xs.compactMap { $0.ok ? $0.name : nil }\n")

    def test_dispatch_sync(self, tmp_path: Path) -> None:
        assert "GL046" in scan_one(tmp_path, "c.swift", "DispatchQueue.main.sync { render() }\n")

    def test_dispatch_async_is_not_flagged(self, tmp_path: Path) -> None:
        assert "GL046" not in scan_one(tmp_path, "d.swift", "DispatchQueue.main.async { render() }\n")

    def test_a_new_session_per_request(self, tmp_path: Path) -> None:
        assert "GL047" in scan_one(tmp_path, "e.swift", "let s = URLSession(configuration: .default)\n")

    def test_shared_session_is_not_flagged(self, tmp_path: Path) -> None:
...
class TestRubyRules:
    ...
    def test_shovel_operator_is_not_flagged(self, tmp_path: Path) -> None:
        # << appends in place; it is the fix, not the smell.
        assert "GL048" not in scan_one(tmp_path, "b.rb", "items.each do |i|\n  out << i.to_s\nend\n")

    def test_query_inside_a_loop(self, tmp_path: Path) -> None:
...
def test_comment_syntax_covers_every_language_with_a_rule() -> None:
...
def test_line_numbers_are_right_when_a_rule_fires_many_times(tmp_path: Path) -> None:
...
def test_line_numbers_survive_a_match_at_the_start_of_a_line(tmp_path: Path) -> None:
    f = write(tmp_path, "q.sql", "-- header\nSELECT * FROM t;\n\nSELECT * FROM u;\n")
    assert [x["line"] for x in greenlint.scan_file(f) if x["rule"] == "GL005"] == [2, 4]


def test_the_language_index_selects_the_same_rules_as_applicable() -> None:
    ...
    for suffix in suffixes:
        ...
        expected = {
            ...
            if greenlint.applicable(r, path) and r["pattern"] is not None and r["id"] not in greenlint.AST_RULE_IDS
        }
        indexed = {r["id"] for r in greenlint.PATTERN_RULES_BY_LANG.get(suffix, ())}
        assert indexed == expected, suffix


def test_scannable_agrees_with_the_rule_table() -> None:
...
def test_ignore_globs_match_the_same_paths_as_fnmatch() -> None:
...
def test_a_file_no_rule_targets_is_not_even_read(tmp_path: Path) -> None:
...
def test_code_shaped_rule_ignores_a_match_inside_a_string(tmp_path: Path) -> None:
    ...
    quoted = ids_for(tmp_path, "b.go", 'func f() {\n\tmsg := "time.Sleep(10 * time.Millisecond)"\n}\n')
    assert "GL002" not in quoted


def test_embedded_content_rules_still_match_inside_strings(tmp_path: Path) -> None:
...
def test_blank_strings_preserves_offsets_and_lines(tmp_path: Path) -> None:
...
def test_blank_strings_handles_escaped_quotes(tmp_path: Path) -> None:
...
def test_blank_strings_does_not_trip_on_an_apostrophe(tmp_path: Path) -> None:
...
def test_blank_strings_leaves_a_language_it_does_not_know(tmp_path: Path) -> None:
...
def test_a_multiline_string_keeps_its_contents_visible(tmp_path: Path) -> None:
